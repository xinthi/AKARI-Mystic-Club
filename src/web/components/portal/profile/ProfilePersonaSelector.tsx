...
function PersonaTagButton({ tag, selected, onClick }: PersonaTagButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`pill-neon px-4 py-2 min-h-[40px] text-xs font-semibold transition-all duration-300 ${
        selected
          ? 'bg-akari-neon-teal/20 text-akari-neon-teal border border-akari-neon-teal/50 shadow-[0_0_12px_rgba(0,246,162,0.3)]'
          : 'bg-akari-cardSoft/50 text-akari-muted border border-akari-neon-teal/20 hover:bg-akari-cardSoft hover:border-akari-neon-teal/40 hover:text-akari-text'
      }`}
    >
  ...
  return (
    <>
      <section className="neon-card neon-hover p-6">
        <h2 className="text-sm uppercase tracking-wider font-semibold text-gradient-neon mb-6">Your Mystic Identity</h2>
        
        {/* Persona Type Toggle */}
        <div className="mb-6">
          <p className="text-xs text-akari-muted mb-3 font-semibold uppercase tracking-wider">I am a/an</p>
          <div className="flex gap-3">
            <button
              onClick={() => handlePersonaTypeChange('individual')}
              className={`flex-1 pill-neon py-3 px-5 min-h-[48px] text-sm font-semibold transition-all duration-300 ${
                localPersonaType === 'individual'
                  ? 'bg-akari-neon-pink/20 text-akari-neon-pink border border-akari-neon-pink/50 shadow-[0_0_12px_rgba(255,16,240,0.3)]'
                  : 'bg-akari-cardSoft/50 text-akari-muted border border-akari-neon-teal/20 hover:border-akari-neon-teal/40'
              }`}
            >
        ...
        {/* Persona Tags */}
        <div className="mb-6">
          <p className="text-xs text-akari-muted mb-3 font-semibold uppercase tracking-wider">
        ...
        {/* Save/Reset buttons */}
        {hasUnsavedChanges && (
          <div className="pt-6 border-t border-akari-neon-teal/20 flex flex-wrap items-center gap-3">
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="pill-neon flex items-center gap-2 px-5 py-2.5 min-h-[44px] bg-gradient-neon-teal text-black font-semibold hover:shadow-akari-glow transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
            >
              {isSaving ? (
                <>
            ...
            <button
              onClick={handleReset}
              disabled={isSaving}
              className="pill-neon px-5 py-2.5 min-h-[44px] bg-akari-cardSoft/50 text-akari-text hover:bg-akari-cardSoft border border-akari-neon-teal/20 hover:border-akari-neon-teal/40 transition-all duration-300 text-sm font-semibold disabled:opacity-50"
            >
              Reset
            </button>
            <p className="text-xs text-akari-neon-pink flex items-center gap-1.5 font-semibold">
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
