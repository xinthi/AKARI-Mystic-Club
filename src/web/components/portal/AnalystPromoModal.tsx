 ...
 * by following @MysticHeros and posting a tweet mentioning them.
 */
...
// Organic tweet messages that sound human and varied
const TWEET_TEMPLATES = [
  `shoutout to @${TARGET_X_HANDLE} for building something actually useful in crypto 🙌`,
  `been using @${TARGET_X_HANDLE}'s tools and the sentiment tracking is lowkey fire 🔥`,
  `@${TARGET_X_HANDLE} helping me stay ahead of the market, pretty impressed ngl`,
  `finally found decent market intel - thanks @${TARGET_X_HANDLE} 💯`,
  `gm! just wanted to say @${TARGET_X_HANDLE} is doing great work for the community`,
  `@${TARGET_X_HANDLE} really knows how to build useful crypto tools, respect`,
  `yo @${TARGET_X_HANDLE} your platform is actually helping me make better trades`,
  `if you're not following @${TARGET_X_HANDLE} you're missing out fr`,
  `@${TARGET_X_HANDLE} the analytics are chef's kiss 👨‍🍳`,
  `big fan of what @${TARGET_X_HANDLE} is building, keep it up!`,
];
...
export function AnalystPromoModal({
  ...
  // Pick a random tweet message (memoized so it doesn't change on every render)
  const randomTweet = useMemo(() => {
    const idx = Math.floor(Math.random() * TWEET_TEMPLATES.length);
    return TWEET_TEMPLATES[idx];
  }, []);
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                {/* Step 2: Tweet */}
                <div className="neon-card neon-hover p-4 border border-akari-neon-violet/20 rounded-xl">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-akari-text font-medium mb-1">
                        ...
                        <code className="px-1.5 py-0.5 rounded bg-akari-cardSoft text-xs text-akari-neon-teal">
                          @{TARGET_X_HANDLE}
                        </code>
                      </p>
                      <p className="text-xs text-akari-muted">
                        Say anything you want! Be creative 🎨
                      </p>
                    </div>
                  </div>
                  {/* Pre-filled tweet suggestion */}
                  <div className="mt-3 p-3 rounded-lg bg-akari-cardSoft/50 border border-akari-neon-violet/10">
                    <p className="text-xs text-akari-muted mb-2">Need inspiration? Try this:</p>
                    <p className="text-sm text-akari-text italic">&quot;{randomTweet}&quot;</p>
                  </div>
                  <a
                    href={`https://x.com/intent/tweet?text=${encodeURIComponent(randomTweet)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-3 w-full pill-neon px-4 py-2 text-xs font-medium bg-akari-neon-violet/10 border border-akari-neon-violet/30 text-akari-neon-violet hover:bg-akari-neon-violet/20 transition-all duration-300 flex items-center justify-center gap-2"
                  >
