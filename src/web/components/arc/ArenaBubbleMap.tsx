 ...
 * Interactive network map with real connections only
 */
...
): Array<Creator & { top: number; left: number; radius: number }> {
  const count = creators.length;
  const positioned: Array<Creator & { top: number; left: number; radius: number }> = [];
 ...
 * Only connects users who are explicitly connected (via connections array)
 */
function calculateConnections(
  positioned: Array<Creator & { top: number; left: number; radius: number }>
): Array<{ from: number; to: number }> {
  const connections: Array<{ from: number; to: number }> = [];
  const usernameToIndex = new Map<string, number>();

  // Build username to index map
  positioned.forEach((creator, index) => {
    usernameToIndex.set(creator.twitter_username.toLowerCase(), index);
  });

  // Only create connections if explicitly defined in connections array
...
export function ArenaBubbleMap({ creators }: ArenaBubbleMapProps) {
  ...
  // Calculate positions
  const positionedCreators = calculatePositions(creators, getRadiusFn);

  // Calculate connections (only real connections)
  const connections = calculateConnections(positionedCreators);

  // Get connected indices for hover/selection highlighting
  const getConnectedIndices = (username: string): number[] => {
    const index = positionedCreators.findIndex(
  ...
  const hoveredIndices = hoveredCreator ? [positionedCreators.findIndex(c => c.twitter_username.toLowerCase() === hoveredCreator.toLowerCase()), ...getConnectedIndices(hoveredCreator)].filter(i => i !== -1) : [];
  const selectedIndices = selectedCreator ? [positionedCreators.findIndex(c => c.twitter_username.toLowerCase() === selectedCreator.toLowerCase()), ...getConnectedIndices(selectedCreator)].filter(i => i !== -1) : [];

  return (
    <div className="relative h-64 w-full rounded-xl bg-gradient-to-b from-slate-950 via-slate-900 to-black border border-white/10 overflow-hidden">
      ...
      {connections.length > 0 && (
        ...
        >
          {connections.map((conn, idx) => {
            const from = positionedCreators[conn.from];
            const to = positionedCreators[conn.to];
      ...
      {/* Creators */}
      {positionedCreators.map((creator, index) => {
        ...
        return (
          <Link
            ...
            className="absolute flex items-center justify-center transition-all duration-300 hover:z-20 group"
            style={{
              top: `${creator.top}%`,
              left: `${creator.left}%`,
              width: `${size}px`,
