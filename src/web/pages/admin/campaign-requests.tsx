 ...
 * 
 * Review and approve/reject campaign requests from projects.
 */
...
export default function AdminCampaignRequestsPage() {
  ...
  const loadRequests = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/campaign-requests?status=${filter}`, {
  ...
  const handleAction = async (requestId: string, action: 'APPROVE' | 'REJECT', createCampaign = false) => {
    try {
      const response = await fetch('/api/admin/campaign-requests', {
        ...
        body: JSON.stringify({ requestId, action, createCampaign }),
      });
      const data = await response.json();
      if (data.ok) {
        setMessage({ 
          type: 'success', 
          text: `${action === 'APPROVE' ? 'Approved' : 'Rejected'}${data.campaign ? ` - Campaign "${data.campaign.name}" created` : ''}` 
        });
        loadRequests();
      } else {
              ...
              <div key={req.id} className="bg-gray-800 rounded-xl p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-gray-400 text-sm">Contact: {req.contactHandle}</p>
                    {req.contactTelegram && <p className="text-gray-500 text-xs">TG: {req.contactTelegram}</p>}
                    {req.contactX && <p className="text-gray-500 text-xs">X: {req.contactX}</p>}
                  </div>
                ...
                <div className="text-xs text-gray-500 mb-4">
                  Requested: {req.requestedStart ? new Date(req.requestedStart).toLocaleDateString() : 'N/A'} 
                  - {req.requestedEnd ? new Date(req.requestedEnd).toLocaleDateString() : 'N/A'}
