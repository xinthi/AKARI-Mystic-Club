/**
 * API Route: POST /api/portal/creator-circles/reject
 * 
 * Reject a circle connection request
 */
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<RejectCircleResponse>
) {
  if (req.method !== 'POST') {
  ...
  try {
    ...
    // Or the sender can cancel their own request
    if (
      circle.circle_member_profile_id !== profileId &&
      circle.initiated_by_profile_id !== profileId
    ) {
      return res.status(403).json({ ok: false, error: 'Not authorized to reject this request' });
    }
    ...
    if (updateError) {
      console.error('[Reject Circle API] Error updating circle:', updateError);
      return res.status(500).json({ ok: false, error: 'Failed to reject connection request' });
    }
    ...
    console.error('[Reject Circle API] Unhandled error:', error);
