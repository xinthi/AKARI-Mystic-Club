 ...
 * 
 * Allows project owners/admins to add/remove team members (admin, moderator roles) for their project.
 */
...
export default function ProjectTeamPage() {
  ...
  const loadProject = useCallback(async () => {
    ...
    try {
      // Get project info from Creator Manager projects API (which includes project details)
      const res = await fetch(`/api/portal/creator-manager/projects`, {
        credentials: 'include',
      });
      ...
      if (res.ok && data.ok && data.projects) {
        // Find the project matching this ID
        const projectData = data.projects.find((p: any) => p.id === projectId);
        if (projectData) {
          setProject({
            id: projectData.id,
          ...
          // Project not found in user's managed projects
          setProject({
            id: projectId,
            name: 'Project',
            display_name: null,
      ...
      // Don't set error here - we'll still show the team management UI
      setProject({
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/portal/projects/team-members?projectId=${encodeURIComponent(projectId)}`, {
        credentials: 'include',
      });
  ...
  useEffect(() => {
    if (projectId && typeof projectId === 'string' && akariUser.isLoggedIn) {
      loadProject();
      loadMembers();
    }
  }, [projectId, akariUser.isLoggedIn, loadProject, loadMembers]);
      ...
      <PortalLayout title="Project Team">
        <div className="px-4 py-4 md:px-6 lg:px-10">
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 text-center">
            <p className="text-slate-400">Please log in to manage your project team.</p>
          </div>
        </div>
      </PortalLayout>
    );
      ...
      <div className="px-4 py-4 md:px-6 lg:px-10">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 text-sm text-slate-400 mb-2">
            <Link href="/portal" className="hover:text-akari-primary transition-colors">
              Portal
            </Link>
            <span>/</span>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  // Basic auth check - the API will handle permission checks
  return { props: {} };
};
